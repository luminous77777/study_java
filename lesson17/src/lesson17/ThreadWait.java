...
public class ThreadWait {
	public static void main(String[] args) throws InterruptedException {
		...
		new Thread(new Cooker(table), "요리사").start(); // 요리사 시작
		new Thread(new Customer(table, "햄버거"), "A").start(); // 손님 A
		new Thread(new Customer(table, "파스타"), "B").start(); // 손님 B
		new Thread(new Customer(table, "피자"), "C").start(); // 손님 C
			...
			while (true) {
				for (int i = 0; i < dishes.size(); i++) {
					if (dish.equals(dishes.get(i))) {
						dishes.remove(i);
						forCook.signal();// 요리사가 잠자고 있으면 깨우기
						return;
					}
				}
				String name = Thread.currentThread().getName();
